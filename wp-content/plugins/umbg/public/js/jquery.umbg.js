 ...
 *
 * Last Change: 2016-03-02 - 1.4.3 - Fix media end fade-out function.
...
(function ( $ ) {
	...
	$.fn.umbg = function ( options ) {
		...
		return this.each( function () {
			...
			// Reset userPause if autoplay is disable.
			if ( opts.autoPlay === 0 ) {
				userPause = 1;
			}
			...
			function ytPlayer() {
				...
				umbgMediaPlayer = '<iframe id="' + opts.mediaPlayerId + '" type="text/html" src="//www.youtube.com/embed/' +
					...
					window.location.origin + '&amp;autoplay=' + opts.autoPlay + '&amp;controls=0&amp;iv_load_policy=3&amp;rel=0' +
			...
			function vimeoPlayer() {
				...
				umbgMediaPlayer = '<iframe id="' + opts.mediaPlayerId + '" src="http://player.vimeo.com/video/' +
					opts.mediaId + '?hd=' + videoQuality + '&amp;title=0&amp;byline=0&amp;portrait=0&amp;autoplay=' + opts.autoPlay +
				...
				function onDmMessageReceived( e ) {
					...
					switch ( dm.event ) {
						...
						case 'play':
							...
							break;
						case 'playing':
							if ( opts.autoPlay === false && dmInitialStart === 1 ) {
								post( 'pause' );
								$playButton.find( 'i' ).removeClass( 'fa-pause' ).addClass( 'fa-play' );
								dmPlaying = 0;
								dmInitialStart = 0;
			...
			function wistiaPlayer() {
				...
				function wistiaInitialize() {
					...
					// Auto playback. (Use 'false' in case autoPlay equals '').
					if ( opts.autoPlay === false ) {
						wistiaEmbed.wistiaApi.pause().time( startTime );
						$playButton.find( 'i' ).removeClass( 'fa-pause' ).addClass( 'fa-play' );
					}
					else {
			...
			function imagePlayer( type ) {
				...
				cssLink.href = myurl + 'umbg-slider/umbg-slider.css?ver=1.3.0';
				...
				// Add umbg-slider.js to the page.
				var script = document.createElement( 'script' );
				script.type = 'text/javascript';
				script.id = 'umbg-slider';
				script.async = true;
				script.src = myurl + 'umbg-slider/jquery.umbg.slider.js';
				// No longer used. Load script below with $.getScript().
				//$( 'head' ).append( script );
				//$( 'body' ).append( script );
				...
				$.getScript( myurl + 'umbg-slider/jquery.umbg.slider.js', function ( data, textStatus, jqxhr ) {
	...
	$.fn.umbg.defaults = {
		...
		'mediaEndFadeOut'            : 0, // When playback ends fade-out the media background. Loop must be disable (0) for this to work.
