<?php
function theo_in_array_r($needle, $haystack, $strict = false) {
    foreach ($haystack as $item) {
        if (($strict ? $item === $needle : $item == $needle) || (is_array($item) && theo_in_array_r($needle, $item, $strict))) {
            return true;
        }
    }
 ...
 function theo_portfolio($group, $layout, $col_layout, $filter_display_status, $custom_class){
  ...
  wp_enqueue_script('packery-parallax-init');
	...
	if($filter_display_status == 'yes')
	{
		...
		foreach($portfolio_categories as $category): 
	        ...
	        { 
		        ...
		        foreach ($item_categories as $index => $project_cat) {
		          ...
		          if($categoryClass == $project_cat->slug && !theo_in_array_r($categoryClass, $filter_items))
		          {
		...
		$filter_list .= '<li><a href="#" data-filter="*" class="filter-active active"><span class="white">'.__('All','signaturelang').'</span></a></li>';
		foreach ($filter_items as $index => $filter_item) 
		{
		  $filter_list .= '<li><a href="#" data-filter=".'.esc_attr($filter_item['slug']).'" ><span class="white font4">'.esc_html($filter_item['name']).'</span></a></li>';
		}

		$filter_markup = '<section id="works-filter-panel" class="works-filter-panel signature-theo dark-bg">
                        <div class="valign">
                          <div class="works-filter-wrap">
                              <ul class="works-filter signature-theo text-center clearfix font4">'.$filter_list.'</ul>
                          </div>
                        </div>
                      </section>';
	}
    ...
    { 
      ...
      if($stellar_ratio_id > 1)
        ...
        if($project_type == 'lightbox_single_image')
        {
          ...
          $portfolio_items .='<div data-stellar-ratio="'.esc_attr($stellar_ratio[$stellar_ratio_id]).'" class="works-item signature-theo '.esc_attr($mouse_icon_class).' '.esc_attr($project_categories).'">
                                ...
                                <a  class="venobox border-'.esc_attr($box_color).'" href="'.esc_url($zoom_image).'">
                                    <div class="works-item-inner">
                                      <h3 class="'.esc_attr($box_color).'">'.esc_html($project_title).'</h3>
                                      <p class="'.esc_attr($box_color).' font3light"><span class="'.esc_attr($box_color).'">'.esc_html($project_caption).'</span></p>
                                    </div>
                                </a>
                              </div>';
        ...
        elseif($project_type == 'lightbox_image_gallery')
        {
            ...
            $portfolio_items .= '<div data-stellar-ratio="'.esc_attr($stellar_ratio[$stellar_ratio_id]).'" class="works-item signature-theo '.esc_attr($mouse_icon_class).' '.esc_attr($project_categories).'">
                                  <img data-no-retina alt="'.esc_attr($project_title).'" title="'.esc_attr($project_title).'" class="img-responsive" src="'.esc_url($project_thumb).'"/>
                                  <a  class="venobox border-'.esc_attr($box_color).'" href="'.esc_url($zoom_image).'" '.$gallery_name.'>
                                      <div class="works-item-inner">
                                        <h3 class="'.esc_attr($box_color).'">'.esc_html($project_title).'</h3>
                                        <p class="'.esc_attr($box_color).' font3light"><span class="'.esc_attr($box_color).'">'.esc_html($project_caption).'</span></p>
                                      </div>
        ...
        elseif($project_type == 'lightbox_yt_video')
        {
          ...
          $portfolio_items .= '<div data-stellar-ratio="'.esc_attr($stellar_ratio[$stellar_ratio_id]).'" class="works-item signature-theo '.esc_attr($mouse_icon_class).' '.esc_attr($project_categories).'">
                                <img data-no-retina alt="'.esc_attr($project_title).'" title="'.esc_attr($project_title).'" class="img-responsive" src="'.esc_url($project_thumb).'"/>
                                <a  class="venobox border-'.esc_attr($box_color).'" data-type="youtube" href="'.esc_url($video_link).'">
                                    <div class="works-item-inner">
                                      <h3 class="'.esc_attr($box_color).'">'.esc_html($project_title).'</h3>
                                      <p class="'.esc_attr($box_color).' font3light"><span class="'.esc_attr($box_color).'">'.esc_html($project_caption).'</span></p>
                                    </div>
        ...
        elseif($project_type == 'lightbox_vimeo_video')
        {
          ...
          $portfolio_items .= '<div data-stellar-ratio="'.esc_attr($stellar_ratio[$stellar_ratio_id]).'" class="works-item signature-theo '.esc_attr($mouse_icon_class).' '.esc_attr($project_categories).'">
                                <img data-no-retina alt="'.esc_attr($project_title).'" title="'.esc_attr($project_title).'" class="img-responsive" src="'.esc_url($project_thumb).'"/>
                                <a  class="venobox border-'.esc_attr($box_color).'" data-type="vimeo" href="'.esc_url($video_link).'">
                                    <div class="works-item-inner">
                                      <h3 class="'.esc_attr($box_color).'">'.esc_html($project_title).'</h3>
                                      <p class="'.esc_attr($box_color).' font3light"><span class="'.esc_attr($box_color).'">'.esc_html($project_caption).'</span></p>
                                    </div>
        ...
        elseif($project_type == 'external_link')
        {
          ...
          $portfolio_items .= '<div data-stellar-ratio="'.esc_attr($stellar_ratio[$stellar_ratio_id]).'" class="works-item signature-theo '.esc_attr($mouse_icon_class).' '.esc_attr($project_categories).'">
                                <img data-no-retina alt="'.esc_attr($project_title).'" title="'.esc_attr($project_title).'" class="img-responsive" src="'.esc_url($project_thumb).'"/>
                                <a  class="border-'.esc_attr($box_color).'" href="'.esc_url($project_url).'">
                                    <div class="works-item-inner">
                                      <h3 class="'.esc_attr($box_color).'">'.esc_html($project_title).'</h3>
                                      <p class="'.esc_attr($box_color).' font3light"><span class="'.esc_attr($box_color).'">'.esc_html($project_caption).'</span></p>
                                    </div>
  ...
  $portfolio_markup = '';
	 ...
	 $portfolio_markup .= '<div id="parallax-random-grid" class="parallax-random-grid signature-theo portfolio-02">
                          <div id="parallax-random-grid-container" class="parallax-grid-container signature-theo">
