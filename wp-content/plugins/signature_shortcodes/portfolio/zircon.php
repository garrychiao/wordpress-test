<?php
function zircon_in_array_r($needle, $haystack, $strict = false) {
    foreach ($haystack as $item) {
        if (($strict ? $item === $needle : $item == $needle) || (is_array($item) && ebert_in_array_r($needle, $item, $strict))) {
            return true;
        }
    }
 ...
 function zircon_portfolio($group, $layout, $col_layout, $filter_display_status, $custom_class){
	...
	if($filter_display_status == 'yes')
	{
		...
		foreach($portfolio_categories as $category): 
	        ...
	        { 
		        ...
		        foreach ($item_categories as $index => $project_cat) {
		          ...
		          if($categoryClass == $project_cat->slug && !zircon_in_array_r($categoryClass, $filter_items))
		          {
		...
		$filter_list .= '<li><a href="#" data-filter="*" class="filter-active active dark"><span>'.__('All','signaturelang').'</span></a></li>';
		foreach ($filter_items as $index => $filter_item) 
		{
		  $filter_list .= '<li><a href="#" data-filter=".'.esc_attr($filter_item['slug']).'" class="dark"><span>'.esc_html($filter_item['name']).'</span></a></li>';
		}

		$filter_markup = '<section id="works-filter-panel" class="works-filter-panel signature-ebert zircon-filter silver-bg">
          						  <div class="valign">
          						    <div class="works-filter-wrap">
          						        <ul class="works-filter signature-ebert text-center clearfix font2">'.$filter_list.'</ul>
          						    </div>
          						  </div>
          						</section>';
	}
  ...
  $overlayclasses = array('ImageOverlayP', 'ImageOverlayMg', 'ImageOverlaySi', 'ImageOverlayAl' );
    ...
    { 
      ...
      if($overlayclass_id > 3)
        ...
        if($project_type == 'lightbox_single_image')
        {
          ...
          $portfolio_items .='<div class="works-item signature-ebert ImageWrapper '.esc_attr($layout_class).' '.esc_attr($mouse_icon_class).' '.esc_attr($project_categories).'">
			                      ...
			                      <a  class="venobox" href="'.esc_url($zoom_image).'">
			                          <div class="works-item-inner '.$overlayclasses[$overlayclass_id].'">
  ...
  $portfolio_markup = '';
	 ...
	 $portfolio_markup .= '<section id="works-container" class="works-container signature-ebert works-masonry-container clearfix works-thumbnails-view">'.$portfolio_items.'</section>';
