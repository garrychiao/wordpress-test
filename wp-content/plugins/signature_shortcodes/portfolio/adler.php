<?php
function adler_in_array_r($needle, $haystack, $strict = false) {
    foreach ($haystack as $item) {
        if (($strict ? $item === $needle : $item == $needle) || (is_array($item) && adler_in_array_r($needle, $item, $strict))) {
            return true;
        }
    }
 ...
 function adler_portfolio($group, $layout, $col_layout, $filter_display_status, $custom_class){
	...
	if($filter_display_status == 'yes')
	{
		...
		foreach($portfolio_categories as $category): 
	        ...
	        { 
		        ...
		        foreach ($item_categories as $index => $project_cat) {
		          ...
		          if($categoryClass == $project_cat->slug && !adler_in_array_r($categoryClass, $filter_items))
		          {
		...
		$filter_markup = '<section class="filter-notification signature-adler pad-top-quarter pad-bottom-quarter pad-left-quarter">
						...
						<section id="works-filter-panel" class="works-filter-panel signature-adler halfheight dark-bg">
						  <div class="valign">
						    <div class="works-filter-wrap">
						        <ul class="works-filter signature-adler text-left clearfix font3">'.$filter_list.'</ul>
						    </div>
						  </div>
						</section>';
	}
    ...
    { 
        ...
        if($project_type == 'lightbox_single_image')
        {
          ...
          $portfolio_items .='<div class="works-item signature-adler ImageWrapper '.esc_attr($layout_class).' '.esc_attr($mouse_icon_class).' '.esc_attr($project_categories).'">
  ...
  if($layout == 'tile')
  {
	 ...
	 $portfolio_markup .= '<section id="works-container" class="works-container signature-adler works-masonry-container clearfix works-thumbnails-view">'.$portfolio_items.'</section>';
  }
  else
  {
   $portfolio_markup .= '<section class="inner-wrap">'.$filter_markup;
   $portfolio_markup .= '<section id="works-container" class="works-container signature-adler works-masonry-container clearfix works-thumbnails-view">'.$portfolio_items.'</section></section>';
  }
