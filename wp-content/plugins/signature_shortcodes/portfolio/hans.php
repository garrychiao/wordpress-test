<?php
function hans_in_array_r($needle, $haystack, $strict = false) {
    foreach ($haystack as $item) {
        if (($strict ? $item === $needle : $item == $needle) || (is_array($item) && hans_in_array_r($needle, $item, $strict))) {
            return true;
        }
    }
 ...
 function hans_portfolio($group, $layout, $col_layout, $filter_display_status, $custom_class){
  ...
  if($filter_display_status == 'yes')
  {
      ...
      while ( $portfolio_loop->have_posts() ) 
          { 
            ...
            foreach ($item_categories as $index => $project_cat) {
              ...
              if($categoryClass == $project_cat->slug && !hans_in_array_r($categoryClass, $filter_items))
              {
    ...
    $filter_markup = '<section id="works-filter-panel" class="works-filter-panel fullheight signature-hans trans-dark-bg">
                        <div class="container valign">
                          <div class="works-filter-wrap">
                              <ul class="works-filter signature-hans text-center clearfix font3">'.$filter_list.'</ul>
                          </div>
                        </div>
                      </section>';
  }
    ...
    { 
      ...
      $project_title = get_the_title();
        ...
        if($project_type == 'lightbox_single_image')
        {
          ...
          $portfolio_items .='<div class="works-item signature-hans ImageWrapper chrome-fix '.esc_attr($layout_class).' '.esc_attr($mouse_icon_class).' '.esc_attr($project_categories).'">
                            ...
                            <a  class="venobox" href="'.esc_url($zoom_image).'">
                                <div class="works-item-inner ImageOverlayNe">
                                  <div class="valign">
                                      <p><span class="white font3">'.esc_attr($project_title).'</span></p>
  ...
  if($layout == 'tile')
  {
   ...
   $portfolio_markup .= '<section id="works-container" class="works-container signature-hans works-masonry-container clearfix works-thumbnails-view">'.$portfolio_items.'</section>';
  }
  else
  {
   $portfolio_markup .= '<section class="inner-wrap">'.$filter_markup;
   $portfolio_markup .= '<section id="works-container" class="works-container signature-hans works-masonry-container clearfix works-thumbnails-view">'.$portfolio_items.'</section></section>';
  }
