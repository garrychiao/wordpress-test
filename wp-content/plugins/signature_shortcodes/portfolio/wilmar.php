<?php
function wilmar_in_array_r($needle, $haystack, $strict = false) {
    foreach ($haystack as $item) {
        if (($strict ? $item === $needle : $item == $needle) || (is_array($item) && wilmar_in_array_r($needle, $item, $strict))) {
            return true;
        }
    }
 ...
 function wilmar_portfolio($group, $layout, $col_layout, $filter_display_status, $custom_class){
	...
	if($filter_display_status == 'yes')
	{
		...
		foreach($portfolio_categories as $category): 
	        ...
	        { 
		        ...
		        foreach ($item_categories as $index => $project_cat) {
		          ...
		          if($categoryClass == $project_cat->slug && !wilmar_in_array_r($categoryClass, $filter_items))
		          {
		...
		$filter_list .= '<li><a href="#" data-filter="*" class="filter-active active"><span class="dark">'.__('All','signaturelang').'</span></a></li>';
		foreach ($filter_items as $index => $filter_item) 
		{
		  $filter_list .= '<li><a href="#" data-filter=".'.esc_attr($filter_item['slug']).'" ><span class="dark">'.esc_html($filter_item['name']).'</span></a></li>';
		}

		$filter_markup = '<section class="filter-notification signature-wilmar white-bg">
                          <a class="font3" href="#" data-show-txt="'.__('Show Categories', 'signature').'" data-hide-txt="'.__('Hide Categories', 'signature').'">'.__('Show Categories', 'signature').'</a>
                      </section>
                      <section id="works-filter-panel" class="works-filter-panel signature-wilmar white-bg">
                          <div class="works-filter-wrap">
                              <ul class="works-filter signature-wilmar text-left clearfix font3">'.$filter_list.'</ul>
                          </div>
                      </section>';
	}
    ...
    { 
        ...
        if($project_type == 'lightbox_single_image')
        {
          ...
          $portfolio_items .='<div class="works-item signature-wilmar GrayScale '.esc_attr($layout_class).' '.esc_attr($mouse_icon_class).' '.esc_attr($project_categories).'">
                                      ...
                                      <p><span class="white black-bg font3">'.esc_html($project_title).'</span></p>
    			                          </div>
    			                      </a>
        				              </div>';
  ...
  $portfolio_markup = '';
	 ...
	 $portfolio_markup .= '<section id="works-container" class="works-container signature-wilmar works-masonry-container clearfix works-thumbnails-view">'.$portfolio_items.'</section>';
