<?php
function claus_in_array_r($needle, $haystack, $strict = false) {
    foreach ($haystack as $item) {
        if (($strict ? $item === $needle : $item == $needle) || (is_array($item) && claus_in_array_r($needle, $item, $strict))) {
            return true;
        }
    }
 ...
 function claus_portfolio($group, $layout, $col_layout, $filter_display_status, $custom_class){
	...
	if($filter_display_status == 'yes')
	{
		...
		foreach($portfolio_categories as $category): 
	        ...
	        { 
		        ...
		        foreach ($item_categories as $index => $project_cat) {
		          ...
		          if($categoryClass == $project_cat->slug && !claus_in_array_r($categoryClass, $filter_items))
		          {
		...
		$filter_list .= '<li><a href="#" data-filter="*" class="filter-active active"><span>'.__('All','signaturelang').'</span></a></li>';
		foreach ($filter_items as $index => $filter_item) 
		{
		  $filter_list .= '<li><a href="#" data-filter=".'.esc_attr($filter_item['slug']).'" ><span>'.esc_html($filter_item['name']).'</span></a></li>';
		}

		$filter_markup = '<section class="filter-notification silver-bg signature-claus pad-top-quarter pad-bottom-quarter pad-left-quarter">
						    <a class="font3bold dark" href="#" data-show-txt="'.__('Show Filters', 'signature').'" data-hide-txt="'.__('Hide Filters', 'signature').'" >'.__('Show Filters', 'signature').'</a>
						</section>

						<section id="works-filter-panel" class="works-filter-panel signature-claus halfheight white-bg">
						  <div class="valign">
						    <div class="works-filter-wrap">
						        <ul class="works-filter signature-claus text-right clearfix font3">'.$filter_list.'</ul>
						    </div>
						  </div>
						</section>';
	}
    ...
    { 
        ...
        if($project_type == 'lightbox_single_image')
        {
          ...
          $portfolio_items .='<div class="works-item signature-claus ImageWrapper '.esc_attr($layout_class).' '.esc_attr($mouse_icon_class).' '.esc_attr($project_categories).'">
			                      ...
			                      <a  class="venobox" href="'.esc_url($zoom_image).'">
			                          <div class="works-item-inner ImageOverlayHe">
			                            <p class="valign text-center"><span class="white font3">'.esc_html($project_title).'</span></p>
			                          </div>
			                      </a>
				              </div>';
  ...
  if($layout == 'tile')
  {
	 ...
	 $portfolio_markup .= '<section id="works-container" class="works-container signature-claus works-masonry-container clearfix works-thumbnails-view">'.$portfolio_items.'</section>';
  }
  else
  {
   $portfolio_markup .= '<section class="inner-wrap">'.$filter_markup;
   $portfolio_markup .= '<section id="works-container" class="works-container signature-claus works-masonry-container clearfix works-thumbnails-view">'.$portfolio_items.'</section></section>';
  }
