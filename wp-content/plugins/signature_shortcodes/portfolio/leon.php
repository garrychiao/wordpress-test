<?php
function leon_in_array_r($needle, $haystack, $strict = false) {
    foreach ($haystack as $item) {
        if (($strict ? $item === $needle : $item == $needle) || (is_array($item) && leon_in_array_r($needle, $item, $strict))) {
            return true;
        }
    }
 ...
 function leon_portfolio($group, $layout, $col_layout, $filter_display_status, $custom_class){
	...
	if($filter_display_status == 'yes')
	{
		...
		foreach($portfolio_categories as $category): 
	        ...
	        { 
		        ...
		        foreach ($item_categories as $index => $project_cat) {
		          ...
		          if($categoryClass == $project_cat->slug && !leon_in_array_r($categoryClass, $filter_items))
		          {
		...
		$filter_markup = '<section id="works-filter-panel" class="works-filter-panel signature-leon dark-bg fullheight">
                        <div class="valign">
                          <div class="works-filter-wrap">
                              <ul class="works-filter signature-leon text-left clearfix font3">'.$filter_list.'</ul>
                          </div>
                        </div>
                      </section>';
	}
    ...
    { 
        ...
        if($project_type == 'lightbox_single_image')
        {
          ...
          $portfolio_items .='<div class="works-item signature-leon BackgroundRR '.esc_attr($layout_class).' '.esc_attr($mouse_icon_class).' '.esc_attr($project_categories).'">
			                      ...
			                      <a  class="venobox" href="'.esc_url($zoom_image).'">
			                          <div class="works-item-inner">
			                            <div class="valign">
                                      <p class="text-center"><span class="white font3bold">'.esc_attr($project_title).'</span></p>
                                    </div>
			                          </div>
			                      </a>
				              </div>';
  ...
  if($layout == 'tile')
  {
	 ...
	 $portfolio_markup .= '<section id="works-container" class="works-container signature-leon works-masonry-container clearfix works-thumbnails-view">'.$portfolio_items.'</section>';
  }
  else
  {
   $portfolio_markup .= '<section class="">'.$filter_markup;
   $portfolio_markup .= '<section id="works-container" class="works-container signature-leon works-masonry-container clearfix works-thumbnails-view">'.$portfolio_items.'</section></section>';
  }
