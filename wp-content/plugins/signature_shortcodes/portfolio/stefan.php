<?php
function stefan_in_array_r($needle, $haystack, $strict = false) {
    foreach ($haystack as $item) {
        if (($strict ? $item === $needle : $item == $needle) || (is_array($item) && stefan_in_array_r($needle, $item, $strict))) {
            return true;
        }
    }
 ...
 function stefan_portfolio($group, $layout, $col_layout, $filter_display_status, $custom_class){
	...
	if($filter_display_status == 'yes')
	{
		...
		foreach($portfolio_categories as $category): 
	        ...
	        { 
		        ...
		        foreach ($item_categories as $index => $project_cat) {
		          ...
		          if($categoryClass == $project_cat->slug && !stefan_in_array_r($categoryClass, $filter_items))
		          {
		...
		{
		  $filter_list .= '<li><a href="#" data-filter=".'.esc_attr($filter_item['slug']).'" ><span class="white">'.esc_html($filter_item['name']).'</span></a></li>';
		}

		$filter_markup = '<section id="works-filter-panel" class="works-filter-panel signature-stefan fullheight dark-bg">
                        <div class="valign">
                          <div class="works-filter-wrap">
                              <ul class="works-filter signature-stefan text-center clearfix font3">'.$filter_list.'</ul>
                          </div>
                        </div>
                      </section>';
	}
    ...
    { 
        ...
        if($project_type == 'lightbox_single_image')
        {
          ...
          $portfolio_items .='<div class="works-item signature-stefan ImageWrapper ContentWrapperB chrome-fix '.esc_attr($layout_class).' '.esc_attr($mouse_icon_class).' '.esc_attr($project_categories).'">
    			                      ...
    			                      <a  class="venobox" href="'.esc_url($zoom_image).'">
    			                          <div class="works-item-inner ContentB">
                                      <div class="content">
                                          <p><span class="dark font3">'.esc_html($project_title).'</span></p>
                                          <h6 class="dark font4">'.esc_html($project_caption).'</h6>
                                      </div>
    			                          </div>
    			                      </a>
        				              </div>';
  ...
  $portfolio_markup = '';
	 ...
	 $portfolio_markup .= '<section id="works-container" class="works-container signature-stefan works-masonry-container clearfix works-thumbnails-view">'.$portfolio_items.'</section>';
