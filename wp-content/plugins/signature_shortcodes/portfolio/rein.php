<?php
function rein_in_array_r($needle, $haystack, $strict = false) {
    foreach ($haystack as $item) {
        if (($strict ? $item === $needle : $item == $needle) || (is_array($item) && rein_in_array_r($needle, $item, $strict))) {
            return true;
        }
    }
 ...
 function rein_portfolio($group, $layout, $col_layout, $filter_display_status, $custom_class){
	...
	if($filter_display_status == 'yes')
	{
		...
		foreach($portfolio_categories as $category): 
	        ...
	        { 
		        ...
		        foreach ($item_categories as $index => $project_cat) {
		          ...
		          if($categoryClass == $project_cat->slug && !rein_in_array_r($categoryClass, $filter_items))
		          {
		...
		{
		  $filter_list .= '<li><a href="#" class="dark" data-filter=".'.esc_attr($filter_item['slug']).'" ><span>'.esc_html($filter_item['name']).'</span></a></li>';
		}

		$filter_markup = '<section id="works-filter-panel" class="works-filter-panel signature-rein">
          						  <div class="valign">
          						    <div class="works-filter-wrap">
          						        <ul class="works-filter signature-rein text-left clearfix font2">'.$filter_list.'</ul>
          						    </div>
          						  </div>
          						</section>';
	}
