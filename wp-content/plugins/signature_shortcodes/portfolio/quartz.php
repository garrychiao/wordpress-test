<?php
function quartz_in_array_r($needle, $haystack, $strict = false) {
    foreach ($haystack as $item) {
        if (($strict ? $item === $needle : $item == $needle) || (is_array($item) && quartz_in_array_r($needle, $item, $strict))) {
            return true;
        }
    }
 ...
 function quartz_portfolio($group, $layout, $col_layout, $filter_display_status, $custom_class){
	...
	if($filter_display_status == 'yes')
	{
		...
		foreach($portfolio_categories as $category): 
	        ...
	        { 
		        ...
		        foreach ($item_categories as $index => $project_cat) {
		          ...
		          if($categoryClass == $project_cat->slug && !quartz_in_array_r($categoryClass, $filter_items))
		          {
		...
		$filter_markup = '<div class="">
                          <article class="text-center offwhite-bg filter-wrap signature-quartz">
                              <ul class="works-filter signature-quartz clearfix font4">'.$filter_list.'</ul>
                          </article>
                      </div>';
	}
    ...
    { 
        ...
        if($project_type == 'lightbox_single_image')
        {
          ...
          $portfolio_items .='<div class="works-item signature-quartz '.esc_attr($layout_class).' '.esc_attr($mouse_icon_class).' '.esc_attr($project_categories).'">
			                      ...
			                      <a  class="venobox" href="'.esc_url($zoom_image).'">
			                          <div class="works-item-inner">
			                            <h3 class="dark font2">'.esc_html($project_title).'</h3>
                                  <p class="color font4"><span class="color font4">'.esc_html($project_caption).'</span></p>
			                          </div>
			                      </a>
				              </div>';
  ...
  $portfolio_markup = '';
	 ...
	 $portfolio_markup .= '<section id="works-container" class="works-container signature-quartz works-masonry-container clearfix works-thumbnails-view">'.$portfolio_items.'</section>';
