...
function velten_blog($item_no, $pagination_opt, $customclass)
{
	$velten_blog_markup = '<section class="journal signature-ebert container-fluid '.esc_attr($customclass).'">
							<div class="row">';
	...
	$velten_blog_list = '';						    
						        ...
						        $velten_blog_list .= '<article class="col-md-6 text-center no-pad">
									                  <section class="news-block signature-ebert velten-news-block '. esc_attr($bg_class) .' img-bg pad-top pad-bottom" data-hover-img="'.esc_url($featured_image_src).'">
	...
	$velten_blog_markup .= $velten_blog_list.'</div>';

	if($pagination_opt == 'yes')
	{
		$velten_blog_markup .= '<div class="row ebert-blog-list-pagination add-top-quarter">
	...
	$velten_blog_markup .= '</section>';

	wp_reset_postdata();


	return $velten_blog_markup;
}
