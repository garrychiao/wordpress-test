...
function moritz_blog($item_no, $pagination_opt, $customclass)
{
	$moritz_blog_markup = '<section class="news-list-wrap signature-leon '.esc_attr($customclass).'">';

	$moritz_blog_list = '<div class="row">';						    
						        ...
						        $moritz_blog_list .= '<article class="col-md-4 no-pad text-left signature-leon news-block '.esc_attr($bg_class).'" data-bg-img="'.esc_url($featured_image_src).'">
								                        <div class="news-head">
	...
	$moritz_blog_markup .= $moritz_blog_list.'</div>';

	if($pagination_opt == 'yes')
	{
		$moritz_blog_markup .= '<div class="row leon-blog-list-pagination text-center dark-bg">
	...
	$moritz_blog_markup .= '</section>';

	wp_reset_postdata();


	return $moritz_blog_markup;
}
